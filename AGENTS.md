...
**Turnstile (bot protection on `/contact`).** Opt-in per deploy: active only when the `TURNSTILE_SITE_KEY` var in `wrangler.jsonc` is non-empty (it ships empty, so local dev, CI and the Vite dev server run without it). To enable: create a widget in the Cloudflare dashboard, run `npx wrangler secret put TURNSTILE_SECRET_KEY` **first**, then set the site key and deploy — key-before-secret makes the form fail closed. When on, the Worker also adds `https://challenges.cloudflare.com` to `script-src`/`frame-src` for HTML responses (see [app/utils/csp.ts](app/utils/csp.ts)). The widget is [app/components/TurnstileWidget/](app/components/TurnstileWidget/); verification lives in [app/utils/turnstile.ts](app/utils/turnstile.ts). Don't test the enforcement with happy-dom's `Request` — it silently strips the forbidden `Origin` header; `action.test.ts` uses the Node environment for that reason.
